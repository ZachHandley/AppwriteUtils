 ...
 * @param apiKey - API key for authentication (optional, can use session auth instead)
 * @param sessionCookie - Session cookie for authentication (optional, alternative to apiKey)
 * @param client - Pre-configured client to use (optional, bypasses client creation)
...
export async function detectAppwriteVersion(
  ...
  try {
    ...
    const tables = new TablesDB(testClient);
    if (dbId) {
...
export async function detectAppwriteVersionCached(
  ...
  const result = await detectAppwriteVersion(endpoint, project, apiKey, sessionCookie, client);
